...
export function useActivatedInkPage({
  pageSize,
  search,
  startId,
  endId,
  ...
  return useSWRInfinite(
    getActivatedInkPageKey(search, startId, endId, startDate, endDate),
    async (
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...[_, search, startId, endId, cursor, startDate, endDate]: InkPageKey
    ) => {
      return backend.getActivatedInkPage({
        limit: pageSize,
        search,
        cursor,
        startId,
...
function getActivatedInkPageKey(
  search?: string,
  startId?: number,
  endId?: number,
  startDate?: Date,
